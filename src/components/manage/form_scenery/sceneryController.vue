<template>
  <div>
    <el-form ref="ruleForm" v-model="item" label-width="85px"  class="demo-ruleForm">
      ...
      <el-form-item :label = "$t('form.price')">
      ...
      <el-form-item :label = "$t('form.kind')">
        <el-input v-model="item.kind" style="width: 20%"></el-input>
      </el-form-item>
...
<script>
  ...
  export default {
    ...
    data(){
      return{
        url:{
          add: this.GLOBAL.addspot,
          update: this.GLOBAL.updatespot
        },
        imgPath:'/spot/',
        sort:[],
        editorOption:{
        ...
        return this.axios({
          ...
          .then(response => {
            for (let i in response.data){
              ...
              if (response.data[i].cnname === '攻略景点') {
